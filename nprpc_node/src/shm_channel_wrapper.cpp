...
ShmChannel::ShmChannel(const std::string& channel_id, bool is_server, bool create)
    ...
    , is_server_(is_server)
{
    ...
    try {
        if (create) {
            ...
            send_ring_ = LockFreeRingBuffer::create(send_ring_name_, RING_BUFFER_SIZE);
            recv_ring_ = LockFreeRingBuffer::create(recv_ring_name_, RING_BUFFER_SIZE);
        } else {
        ...
        // Create eventfd for libuv polling (used for signaling data availability)
...
Napi::Object ShmChannelWrapper::Init(Napi::Env env, Napi::Object exports) {
    Napi::Function func = DefineClass(env, "ShmChannel", {
...
Napi::Value ShmChannelWrapper::Send(const Napi::CallbackInfo& info) {
    ...
    bool success = channel_->send(data.Data(), static_cast<uint32_t>(data.ByteLength()));
...
Napi::Value ShmChannelWrapper::TryReceive(const Napi::CallbackInfo& info) {
    ...
    static thread_local std::vector<uint8_t> recv_buffer(ShmChannel::MAX_MESSAGE_SIZE);
    
    int32_t bytes_read = channel_->try_receive(recv_buffer.data(), recv_buffer.size());
...
Napi::Value ShmChannelWrapper::StartPolling(const Napi::CallbackInfo& info) {
    ...
    uv_poll_start(poll_handle_, UV_READABLE, [](uv_poll_t* handle, int status, int events) {
        auto* self = static_cast<ShmChannelWrapper*>(handle->data);
        if (!self->polling_ || !self->channel_) return;
        ...
        if (events & UV_READABLE) {
            ...
            self->tsfn_.NonBlockingCall([](Napi::Env env, Napi::Function callback) {
                callback.Call({});
            });
        }
    });
