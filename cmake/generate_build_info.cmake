function(get_git_commit dir override_commit out_var)
  if(NOT "${override_commit}" STREQUAL "")
    set(${out_var} "${override_commit}" PARENT_SCOPE)
    return()
  endif()
...
function(get_override_commit name out_var)
  if("${THIRD_PARTY_GIT_COMMITS}" STREQUAL "")
    set(${out_var} "" PARENT_SCOPE)
    return()
  endif()

  string(REPLACE "|" ";" commit_entries "${THIRD_PARTY_GIT_COMMITS}")
  foreach(entry IN LISTS commit_entries)
    if(entry MATCHES "^([^=]+)=(.*)$" AND CMAKE_MATCH_1 STREQUAL "${name}")
...
get_git_commit("${ROOT_DIR}" "${PROJECT_GIT_COMMIT}" project_commit)

set(third_party_entries "")
foreach(dir IN LISTS THIRD_PARTY_DIRS)
  get_override_commit("${dir}" dependency_commit_override)
  get_git_commit("${ROOT_DIR}/third_party/${dir}" "${dependency_commit_override}" dependency_commit)
