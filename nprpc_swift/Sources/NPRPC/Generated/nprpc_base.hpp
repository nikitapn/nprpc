...
// Module export macro
#ifdef NPRPC_EXPORTS
#  define NPRPC_BASE_API NPRPC_EXPORT_ATTR
#else
#  define NPRPC_BASE_API NPRPC_IMPORT_ATTR
#endif
...
class StreamInit_Direct {
  ::nprpc::flat_buffer& buffer_;
  const std::uint32_t offset_;

  auto& base() noexcept { return *reinterpret_cast<StreamInit*>(reinterpret_cast<std::byte*>(buffer_.data().data()) + offset_); }
  auto const& base() const noexcept { return *reinterpret_cast<const StreamInit*>(reinterpret_cast<const std::byte*>(buffer_.data().data()) + offset_); }
public:
  uint32_t offset() const noexcept { return offset_; }
  void* __data() noexcept { return (void*)&base(); }
  StreamInit_Direct(::nprpc::flat_buffer& buffer, std::uint32_t offset)
    : buffer_(buffer)
    , offset_(offset)
  {
  }
  const uint64_t& stream_id() const noexcept { return base().stream_id;}
  uint64_t& stream_id() noexcept { return base().stream_id;}
  const uint16_t& poa_idx() const noexcept { return base().poa_idx;}
  uint16_t& poa_idx() noexcept { return base().poa_idx;}
  const uint8_t& interface_idx() const noexcept { return base().interface_idx;}
  uint8_t& interface_idx() noexcept { return base().interface_idx;}
  const uint64_t& object_id() const noexcept { return base().object_id;}
  uint64_t& object_id() noexcept { return base().object_id;}
  const uint8_t& func_idx() const noexcept { return base().func_idx;}
  uint8_t& func_idx() noexcept { return base().func_idx;}
};
} // namespace flat
...
class StreamChunk_Direct {
  ::nprpc::flat_buffer& buffer_;
  const std::uint32_t offset_;

  auto& base() noexcept { return *reinterpret_cast<StreamChunk*>(reinterpret_cast<std::byte*>(buffer_.data().data()) + offset_); }
  auto const& base() const noexcept { return *reinterpret_cast<const StreamChunk*>(reinterpret_cast<const std::byte*>(buffer_.data().data()) + offset_); }
public:
  uint32_t offset() const noexcept { return offset_; }
  void* __data() noexcept { return (void*)&base(); }
  StreamChunk_Direct(::nprpc::flat_buffer& buffer, std::uint32_t offset)
    : buffer_(buffer)
    , offset_(offset)
  {
  }
  const uint64_t& stream_id() const noexcept { return base().stream_id;}
  uint64_t& stream_id() noexcept { return base().stream_id;}
  const uint64_t& sequence() const noexcept { return base().sequence;}
  uint64_t& sequence() noexcept { return base().sequence;}
  void data(std::uint32_t elements_size) { new (&base().data) ::nprpc::flat::Vector<uint8_t>(buffer_, elements_size); }
  auto data_d() noexcept { return ::nprpc::flat::Vector_Direct1<uint8_t>(buffer_, offset_ + offsetof(StreamChunk, data)); }
  auto data() noexcept { return (::nprpc::flat::Span<uint8_t>)base().data; }
  const auto data() const noexcept { return (::nprpc::flat::Span<const uint8_t>)base().data; }
  const uint32_t& window_size() const noexcept { return base().window_size;}
  uint32_t& window_size() noexcept { return base().window_size;}
};
} // namespace flat
...
class StreamComplete_Direct {
  ::nprpc::flat_buffer& buffer_;
  const std::uint32_t offset_;

  auto& base() noexcept { return *reinterpret_cast<StreamComplete*>(reinterpret_cast<std::byte*>(buffer_.data().data()) + offset_); }
  auto const& base() const noexcept { return *reinterpret_cast<const StreamComplete*>(reinterpret_cast<const std::byte*>(buffer_.data().data()) + offset_); }
public:
  uint32_t offset() const noexcept { return offset_; }
  void* __data() noexcept { return (void*)&base(); }
  StreamComplete_Direct(::nprpc::flat_buffer& buffer, std::uint32_t offset)
    : buffer_(buffer)
    , offset_(offset)
  {
  }
  const uint64_t& stream_id() const noexcept { return base().stream_id;}
  uint64_t& stream_id() noexcept { return base().stream_id;}
  const uint64_t& final_sequence() const noexcept { return base().final_sequence;}
  uint64_t& final_sequence() noexcept { return base().final_sequence;}
};
} // namespace flat
...
struct StreamError {
  uint64_t stream_id;
  uint32_t error_code;
  ::nprpc::flat::Vector<uint8_t> error_data;
};

class StreamError_Direct {
  ::nprpc::flat_buffer& buffer_;
  const std::uint32_t offset_;

  auto& base() noexcept { return *reinterpret_cast<StreamError*>(reinterpret_cast<std::byte*>(buffer_.data().data()) + offset_); }
  auto const& base() const noexcept { return *reinterpret_cast<const StreamError*>(reinterpret_cast<const std::byte*>(buffer_.data().data()) + offset_); }
public:
  uint32_t offset() const noexcept { return offset_; }
  void* __data() noexcept { return (void*)&base(); }
  StreamError_Direct(::nprpc::flat_buffer& buffer, std::uint32_t offset)
    : buffer_(buffer)
    , offset_(offset)
  {
  }
  const uint64_t& stream_id() const noexcept { return base().stream_id;}
  uint64_t& stream_id() noexcept { return base().stream_id;}
  const uint32_t& error_code() const noexcept { return base().error_code;}
  uint32_t& error_code() noexcept { return base().error_code;}
  void error_data(std::uint32_t elements_size) { new (&base().error_data) ::nprpc::flat::Vector<uint8_t>(buffer_, elements_size); }
  auto error_data_d() noexcept { return ::nprpc::flat::Vector_Direct1<uint8_t>(buffer_, offset_ + offsetof(StreamError, error_data)); }
  auto error_data() noexcept { return (::nprpc::flat::Span<uint8_t>)base().error_data; }
  const auto error_data() const noexcept { return (::nprpc::flat::Span<const uint8_t>)base().error_data; }
};
} // namespace flat
...
class StreamCancel_Direct {
  ::nprpc::flat_buffer& buffer_;
  const std::uint32_t offset_;

  auto& base() noexcept { return *reinterpret_cast<StreamCancel*>(reinterpret_cast<std::byte*>(buffer_.data().data()) + offset_); }
  auto const& base() const noexcept { return *reinterpret_cast<const StreamCancel*>(reinterpret_cast<const std::byte*>(buffer_.data().data()) + offset_); }
public:
  uint32_t offset() const noexcept { return offset_; }
  void* __data() noexcept { return (void*)&base(); }
  StreamCancel_Direct(::nprpc::flat_buffer& buffer, std::uint32_t offset)
    : buffer_(buffer)
    , offset_(offset)
  {
  }
