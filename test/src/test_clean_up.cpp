...
void run()
{
  ...
  public:
    ...
    void UpdatePosition(uint32_t player_id,
                        test_udp::flat::Vec3_Direct pos,
                        test_udp::flat::Quaternion_Direct rot) override
    {
      // Implementation of UpdatePosition
      ctl.ack->Confirm({}, "UpdatePosition ACK");
    }

    void FireWeapon(uint32_t player_id,
                    uint8_t weapon_id,
    ...
    void PlaySound(uint16_t sound_id,
                   test_udp::flat::Vec3_Direct position,
    ...
    void SpawnEntity1(uint16_t entity_type,
                      test_udp::flat::Vec3_Direct position) override
    {
  ...
  public:
    ...
    ~TestUdpAckImpl() { std::cout << "TestUdpAckImpl destroyed" << std::endl; }
  } ack_servant; // Local
  ...
  try {
    ...
    // Important: Do not use SHARED_MEMORY for the callback object, as the
    // server and client both shares the same g_shared_memory_listener
    // object, which can lead to conflicts and unexpected behavior.
    auto oid = poa->activate_object(&ack_servant,
