...
{
  stream_promise_ =
      std::make_shared<std::promise<std::unique_ptr<StreamType>>>();
...
template <typename Type>
void AsyncConnector<Type>::on_resolve(const beast::error_code& ec,
                                      tcp::resolver::results_type results)
{
  if (ec) {
    cleanup();
    stream_promise_->set_exception(std::make_exception_ptr(
  ...
  auto asyncon = [this]<typename EndpointType>(EndpointType& ep) {
    ...
    auto stream =
        std::make_unique<beast_tcp_stream_strand>(net::make_strand(ioc_));

    // Set a timeout on the operation
    stream->expires_after(std::chrono::seconds(6));

    stream->async_connect(
        ep, [stream = std::move(stream), self = this->shared_from_this()](
                const beast::error_code& ec,
                tcp::resolver::results_type::endpoint_type endpoint) mutable {
    ...
    std::unique_ptr<beast_tcp_stream_strand> stream)
{
  if (ec) {
    cleanup();
    stream_promise_->set_exception(std::make_exception_ptr(nprpc::Exception(
        "Error while connecting to " + host_ + " - " + ec.message())));
    return;
  }
  ...
  if constexpr (std::is_same_v<Type, type_ws>) {
    ...
    ws->async_handshake(
        host_, "/",
        net::bind_executor(
            ioc_, [self = this->shared_from_this(),
                   ws = std::move(ws)](const beast::error_code& ec) mutable {
...
template <typename Type>
void AsyncConnector<Type>::on_ws_handshake(const beast::error_code& ec,
...
template <typename Type>
void AsyncConnector<Type>::on_ssl_ws_handshake(const beast::error_code& ec,
                                               std::unique_ptr<ssl_ws>&& ws)
{
  if constexpr (std::is_same_v<Type, type_wss>) {
    if (ec) {
      cleanup();
      stream_promise_->set_exception(std::make_exception_ptr(std::runtime_error(
          "SSL WebSocket handshake failed: " + ec.message())));
      return;
    }
...
template <typename Type> void AsyncConnector<Type>::cleanup()
{
