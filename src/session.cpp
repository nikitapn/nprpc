...
std::optional<ObjectGuard> get_object(
  SessionContext& ctx, uint16_t poa_idx, uint64_t object_id)
{
  do {
...
void Session::handle_request(
  flat_buffer& rx_buffer, flat_buffer& tx_buffer)
{
  ...
  switch (header->msg_id) {
    case MessageId::FunctionCall: {
      ...
      if (g_cfg.debug_level >= DebugLevel::DebugLevel_EveryCall) {
        NPRPC_LOG_INFO(
          "[nprpc] Handle Request: FunctionCall. request_id: {}, "
          "interface_idx: {}, fn_idx: {}, poa_idx: {}, oid: {}",
          header->request_id,
          (uint32_t)ch.interface_idx(),
          (uint32_t)ch.function_idx(),
          ch.poa_idx(),
          ch.object_id());
      }
        ...
        if (auto real_obj = (*obj).get(); real_obj) {
          ...
          try {
            ...
            NPRPC_LOG_ERROR(
              ...
              e.what());
            // TODO: find out why Web client does not handle Error_BadInput
            // properly
