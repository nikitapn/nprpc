...
struct Http3Stream {
  ...
  std::string
    response_content_type;  // Store response content-type for lifetime
...
class Http3Connection : public std::enable_shared_from_this<Http3Connection>
{
 ...
 private:
  ...
  static int on_recv_client_initial(
  ...
  static int on_recv_crypto_data(
    ngtcp2_conn*            conn,
    ...
    void*                   user_data)
  {
    ...
    int rv = ngtcp2_crypto_recv_crypto_data_cb(
      conn, crypto_level, offset, data, datalen, user_data);
...
Http3Connection::Http3Connection(
  ...
  SSL_CTX*          ssl_ctx)
    : server_(server),
      ...
      version_(version)
{
  // dcid_ stores the client's source CID - this is what ngtcp2_conn_server_new
  // expects as its 'dcid' parameter (the CID that appears in client Initial as
  // Source CID)
  dcid_ = *client_scid;
...
bool Http3Connection::init()
{
  ...
  if (ngtcp2_crypto_generate_stateless_reset_token(params.stateless_reset_token,
                                                   g_static_secret.data(),
                                                   g_static_secret.size(),
                                                   &scid_) != 0) {
  ...
  int rv = ngtcp2_conn_server_new(&conn_,
                                  &dcid_,
                                  &scid_,
                                  &path,
                                  version_,
...
int Http3Connection::on_read(
  const uint8_t* data, size_t len, const ngtcp2_pkt_info* pi)
{
  if (closed_) {
    return -1;
  }
  ...
  if (rv != 0) {
    switch (rv) {
      case NGTCP2_ERR_DRAINING:
        ...
        NPRPC_HTTP3_TRACE(
          "Connection entering draining period (client closed)");
  ...
  for (;;) {
    ...
    if (httpconn_ && ngtcp2_conn_get_max_data_left(conn_)) {
      sveccnt = nghttp3_conn_writev_stream(
        httpconn_, &stream_id, &fin, vec.data(), vec.size());
      if (sveccnt < 0) {
    ...
    NPRPC_HTTP3_TRACE(
      "on_write: calling ngtcp2_conn_writev_stream, stream_id={}, sveccnt={}",
      stream_id,
      sveccnt);

    auto nwrite =
      ngtcp2_conn_writev_stream(conn_,
                                &ps.path,
                                &pi,
                                send_buf_.data(),
    ...
    NPRPC_HTTP3_TRACE(
      "on_write: ngtcp2_conn_writev_stream returned nwrite={}, ndatalen={}",
      nwrite,
      ndatalen);
    ...
    if (nwrite < 0) {
      ...
      ngtcp2_ccerr_set_liberr(
        &last_error_, static_cast<int>(nwrite), nullptr, 0);
      return handle_error();
    }
...
#endif
    server_->send_packet(
      remote_ep_, send_buf_.data(), static_cast<size_t>(nwrite));
  }
...
int Http3Connection::setup_httpconn()
{
  ...
  rv = nghttp3_conn_bind_qpack_streams(
    httpconn_, qpack_enc_stream_id, qpack_dec_stream_id);
  if (rv != 0) {
...
Http3Stream* Http3Connection::find_stream(
  int64_t stream_id)
{
...
Http3Stream* Http3Connection::create_stream(
  int64_t stream_id)
{
...
void Http3Connection::remove_stream(
  int64_t stream_id)
{
  streams_.erase(stream_id);
}

int Http3Connection::recv_stream_data(
  uint32_t flags, int64_t stream_id, const uint8_t* data, size_t datalen)
{
  ...
  auto nconsumed =
    nghttp3_conn_read_stream2(httpconn_,
                              stream_id,
                              data,
                              datalen,
  ...
  ngtcp2_conn_extend_max_stream_offset(
    conn_, stream_id, static_cast<uint64_t>(nconsumed));
  ngtcp2_conn_extend_max_offset(conn_, static_cast<uint64_t>(nconsumed));

  return 0;
}

int Http3Connection::acked_stream_data_offset(
...
void Http3Connection::extend_max_remote_streams_bidi(
  uint64_t max_streams)
{
  if (httpconn_) {
...
void Http3Connection::extend_max_stream_data(
...
void Http3Connection::http_begin_headers(
  int64_t stream_id)
{
...
void Http3Connection::http_recv_header(
  Http3Stream* stream, int32_t token, nghttp3_rcbuf* name, nghttp3_rcbuf* value)
{
...
int Http3Connection::http_end_headers(
  Http3Stream* stream)
{
...
int Http3Connection::http_end_stream(
  Http3Stream* stream)
{
  return start_response(stream);
}

void Http3Connection::http_stream_close(
...
int Http3Connection::start_response(
  Http3Stream* stream)
{
  ...
  if (stream->method == "GET" || stream->method == "HEAD") {
    ...
    if (g_cfg.ssr_enabled &&
      ...
      NPRPC_HTTP3_TRACE(
        "Forwarding to SSR: {} {}", stream->method, stream->path);
      ...
      auto ssr_response =
        nprpc::impl::forward_to_ssr(stream->method,
                                    url,
                                    filtered_headers,
      ...
      if (ssr_response) {
        NPRPC_HTTP3_TRACE("SSR response: {} ({} bytes)",
                          ssr_response->status_code,
                          ssr_response->body.size());
        ...
        return send_dynamic_response(stream,
                                     ssr_response->status_code,
                                     content_type,
    ...
    // Method not allowed
    return send_static_response(stream,
                                405,
                                "text/html",
...
int Http3Connection::send_cached_response(
  Http3Stream* stream, unsigned int status_code, CachedFileGuard cached_file)
{
  NPRPC_HTTP3_TRACE(
    "Sending cached response: {} Content-Type: {} Body length: {}",
    status_code,
    cached_file->content_type(),
    cached_file->size());
  ...
  int rv = nghttp3_conn_submit_response(
    httpconn_, stream->stream_id, nva.data(), nva.size(), &dr);
  if (rv != 0) {
...
// RPC Handling
//==============================================================================
int Http3Connection::handle_rpc_request(
  Http3Stream* stream)
{
  if (stream->request_body.empty())
    return send_static_response(
      stream, 400, "text/plain", "Empty request body");
  ...
  try {
    ...
    if (!process_http_rpc(server_->io_context(), request_body, response_body))
      return send_static_response(
        stream, 500, "text/plain", "RPC processing failed");
    ...
    return send_dynamic_response(
      stream, 200, "application/octet-stream", std::move(response_body));
...
int Http3Connection::send_static_response(
  Http3Stream*     stream,
  unsigned int     status_code,
  std::string_view content_type,
  std::string_view body)
...
int Http3Connection::send_dynamic_response(
  Http3Stream*     stream,
  unsigned int     status_code,
  std::string_view content_type,
  std::string&&    body)
{
  ...
  nva[2] = {
    .name = reinterpret_cast<uint8_t*>(const_cast<char*>("content-type")),
    .value =
      reinterpret_cast<const uint8_t*>(stream->response_content_type.data()),
    .namelen  = 12,
...
int Http3Connection::handle_error()
{
  ...
  if (!conn_close_buf_.empty()) {
    server_->send_packet(
      remote_ep_, conn_close_buf_.data(), conn_close_buf_.size());
  }
...
int Http3Connection::start_closing_period()
{
  ...
  ngtcp2_pkt_info pi;
  auto            n = ngtcp2_conn_write_connection_close(conn_,
                                              &ps.path,
                                              &pi,
                                              conn_close_buf_.data(),
...
// Debugging
//==============================================================================
void Http3Connection::print_ssl_state(
  std::string_view prefix)
{
  ...
  if (ssl) {
    NPRPC_HTTP3_TRACE(
      "SSL [{}]: state: {}", prefix, SSL_state_string_long(ssl));
    int ssl_err = SSL_get_error(ssl, 0);
    if (ssl_err != SSL_ERROR_NONE) {
      NPRPC_HTTP3_TRACE(
        "SSL [{}]: error: {}", prefix, ERR_error_string(ssl_err, nullptr));
    }
    // Check pending errors
    unsigned long err;
    while ((err = ERR_get_error()) != 0) {
      NPRPC_HTTP3_TRACE(
        "SSL [{}]: pending error: {}", prefix, ERR_error_string(err, nullptr));
    }
  }
}
...
int Http3Connection::on_recv_stream_data(
  ngtcp2_conn*   conn,
  uint32_t       flags,
  int64_t        stream_id,
  uint64_t       offset,
...
int Http3Connection::on_acked_stream_data_offset(
  ngtcp2_conn* conn,
  int64_t      stream_id,
  uint64_t     offset,
  uint64_t     datalen,
...
int Http3Connection::on_stream_open(
...
int Http3Connection::on_stream_close(
  ngtcp2_conn* conn,
  uint32_t     flags,
  int64_t      stream_id,
  uint64_t     app_error_code,
  ...
  if (h->httpconn_) {
    ...
    if (rv != 0 && rv != NGHTTP3_ERR_STREAM_NOT_FOUND) {
      std::cerr << "[HTTP/3][E] nghttp3_conn_close_stream: "
                << nghttp3_strerror(rv) << std::endl;
      ngtcp2_ccerr_set_application_error(
        &h->last_error_, nghttp3_err_infer_quic_app_error_code(rv), nullptr, 0);
      return NGTCP2_ERR_CALLBACK_FAILURE;
    }
  }
...
int Http3Connection::on_extend_max_remote_streams_bidi(
...
int Http3Connection::on_extend_max_stream_data(
  ngtcp2_conn* conn,
  int64_t      stream_id,
  uint64_t     max_data,
  void*        user_data,
...
int Http3Connection::on_recv_tx_key(
  ngtcp2_conn* conn, ngtcp2_encryption_level level, void* user_data)
{
...
int Http3Connection::on_get_new_connection_id(
  ngtcp2_conn* conn,
  ngtcp2_cid*  cid,
  uint8_t*     token,
  size_t       cidlen,
...
int Http3Connection::on_remove_connection_id(
...
void Http3Connection::on_rand(
  uint8_t* dest, size_t destlen, const ngtcp2_rand_ctx* rand_ctx)
{
...
int Http3Connection::http_acked_stream_data_cb(
  nghttp3_conn* conn,
  int64_t       stream_id,
  uint64_t      datalen,
  void*         user_data,
...
int Http3Connection::http_recv_data_cb(
  nghttp3_conn*  conn,
  int64_t        stream_id,
  const uint8_t* data,
  size_t         datalen,
  ...
  void*          stream_user_data)
{
  ...
  if (stream) {
    stream->request_body.insert(
      stream->request_body.end(), data, data + datalen);
  }
...
int Http3Connection::http_deferred_consume_cb(
  nghttp3_conn* conn,
  int64_t       stream_id,
  size_t        nconsumed,
  void*         user_data,
...
int Http3Connection::http_begin_headers_cb(
  nghttp3_conn* conn,
  int64_t       stream_id,
  void*         user_data,
  void*         stream_user_data)
...
int Http3Connection::http_recv_header_cb(
  nghttp3_conn*  conn,
  int64_t        stream_id,
  int32_t        token,
  nghttp3_rcbuf* name,
...
int Http3Connection::http_end_headers_cb(
  nghttp3_conn* conn,
  int64_t       stream_id,
  int           fin,
  void*         user_data,
...
int Http3Connection::http_end_stream_cb(
  nghttp3_conn* conn,
  int64_t       stream_id,
  void*         user_data,
  void*         stream_user_data)
...
int Http3Connection::http_stop_sending_cb(
  nghttp3_conn* conn,
  int64_t       stream_id,
  uint64_t      app_error_code,
  void*         user_data,
...
int Http3Connection::http_reset_stream_cb(
  nghttp3_conn* conn,
  int64_t       stream_id,
  uint64_t      app_error_code,
  void*         user_data,
...
nghttp3_ssize Http3Connection::http_read_data_cb(
  nghttp3_conn* conn,
  int64_t       stream_id,
  nghttp3_vec*  vec,
  size_t        veccnt,
...
void Http3Server::associate_cid(
  const ngtcp2_cid* cid, std::shared_ptr<Http3Connection> conn)
{
...
void Http3Server::dissociate_cid(
  const ngtcp2_cid* cid)
{
  std::string     key = cid_to_string(cid);
  std::lock_guard lock(mutex_);
...
void Http3Server::remove_connection(
  std::shared_ptr<Http3Connection> conn)
{
  std::lock_guard lock(mutex_);
...
void Http3Server::handle_packet(
  const boost::asio::ip::udp::endpoint& remote_ep,
  const uint8_t*                        data,
  size_t                                len)
{
...
int Http3Server::send_version_negotiation(
  const boost::asio::ip::udp::endpoint& remote_ep, const ngtcp2_version_cid& vc)
{
  ...
  auto nwrite = ngtcp2_pkt_write_version_negotiation(buf.data(),
                                                     buf.size(),
                                                     std::rand() % 256,
                                                     vc.scid,
                                                     vc.scidlen,
...
std::shared_ptr<Http3Connection> Http3Server::find_connection(
