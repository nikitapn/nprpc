...
bool Http3Server::start()
{
  ...
  api_ = MsH3ApiOpen();
  if (!api_) {
    NPRPC_LOG_ERROR("[HTTP/3] Failed to initialize msh3 API");
    return false;
  }
  ...
  if (!config_) {
    NPRPC_LOG_ERROR("[HTTP/3] Failed to create configuration");
    MsH3ApiClose(api_);
    api_ = nullptr;
    return false;
  }
  ...
  if (MSH3_FAILED(status)) {
    NPRPC_LOG_ERROR("[HTTP/3] Failed to load credentials, status=0x{:x}",
                    status);
  ...
  if (!listener_) {
    NPRPC_LOG_ERROR("[HTTP/3] Failed to create listener");
    MsH3ConfigurationClose(config_);
    MsH3ApiClose(api_);
    config_ = nullptr;
    api_ = nullptr;
                                        ...
                                        const char* server_name)
{
  ...
  if (MSH3_FAILED(status)) {
    NPRPC_LOG_ERROR("[HTTP/3] Failed to configure connection");
...
void Http3Server::handle_rpc_request(Http3Request* req)
{
  ...
  try {
    ...
    NPRPC_LOG_ERROR("[HTTP/3] RPC exception: {}", e.what());
...
NPRPC_API void init_http3_server(boost::asio::io_context& ioc)
{
  ...
  if (!g_http3_server->start()) {
    ...
    NPRPC_LOG_INFO("[HTTP/3] Server started successfully on port {}",
