...
CachedFileGuard HttpFileCache::get(const std::filesystem::path& path) {
    ...
    // Try read-lock first (fast path for cache hit)
    {
        ...
        if (it != cache_.end()) {
            ...
            // Check if file is stale (without lock upgrade - just return cached version)
            // Staleness is checked with timestamp, actual reload happens on next access
...
void HttpFileCache::put(const std::filesystem::path& path, std::shared_ptr<CachedFile> file) {
    auto canonical_path = path.lexically_normal().string();
...
HttpFileCache::Stats HttpFileCache::stats() const {
    ...
    return Stats{
        ...
        .eviction_skips = eviction_skips_.load(std::memory_order_relaxed)
    };
}
...
bool HttpFileCache::is_stale(CacheEntry& entry, const std::filesystem::path& path) {
