...
HQUIC QuicApi::create_configuration(
  const char* alpn, bool is_server, const char* cert_file, const char* key_file)
{
  ...
  QUIC_STATUS status        = api_->ConfigurationOpen(registration_,
                                               &alpn_buf,
                                               1,
                                               &settings,
                                               sizeof(settings),
...
void QuicConnection::async_connect(
  const std::string& host, uint16_t port, ConnectCallback callback)
{
  ...
  // Start connection
  status = quic.api()->ConnectionStart(connection_,
                                       configuration_,
...
void QuicConnection::send_receive(
  flat_buffer& buffer, uint32_t timeout_ms)
{
  ...
  QUIC_STATUS status =
    quic.api()->StreamSend(stream_, send_buf, 1, QUIC_SEND_FLAG_NONE, send_buf);
...
void QuicConnection::async_send_stream(
  const uint8_t* data, size_t len, std::function<void(bool)> callback)
{
  if (!connected_ || !stream_) {
    boost::asio::post(ioc_, [callback]() { callback(false); });
    return;
...
bool QuicConnection::send_datagram(
  const uint8_t* data, size_t len)
{
  NPRPC_QUIC_DEBUG_LOG(std::format(
    "send_datagram called, len={}, connected={}, datagram_enabled={}",
    len,
    (bool)connected_,
  ...
  if (len > max_datagram_size_) {
    NPRPC_QUIC_DEBUG_LOG(std::format(
      "send_datagram: size {} exceeds max {}", len, max_datagram_size_));
    return false;
  }
  ...
  QUIC_STATUS status = quic.api()->DatagramSend(connection_,
                                                send_buf,
                                                1,
                                                QUIC_SEND_FLAG_NONE,
...
void QuicConnection::set_receive_callback(
...
void QuicConnection::handle_connection_event(
  QUIC_CONNECTION_EVENT* event)
{
  switch (event->Type) {
    case QUIC_CONNECTION_EVENT_CONNECTED: {
      ...
      auto&       quic   = QuicApi::instance();
      QUIC_STATUS status = quic.api()->StreamOpen(connection_,
    ...
    case QUIC_CONNECTION_EVENT_DATAGRAM_STATE_CHANGED: {
      ...
      NPRPC_QUIC_DEBUG_LOG(
        std::format("DATAGRAM_STATE_CHANGED: SendEnabled={}, MaxSendLength={}",
                    (int)datagram_send_enabled_.load(),
                    max_datagram_size_));
      break;
    }
...
void QuicConnection::handle_stream_event(
...
QuicServerConnection::QuicServerConnection(
  boost::asio::io_context& ioc, HQUIC connection, HQUIC configuration)
...
void QuicServerConnection::set_message_callback(
...
void QuicServerConnection::set_datagram_callback(
...
bool QuicServerConnection::send(
...
void QuicServerConnection::handle_connection_event(
  QUIC_CONNECTION_EVENT* event)
{
  NPRPC_QUIC_DEBUG_LOG(
  ...
  switch (event->Type) {
    ...
    case QUIC_CONNECTION_EVENT_DATAGRAM_RECEIVED: {
      ...
      if (datagram_callback_) {
        auto& buf = event->DATAGRAM_RECEIVED.Buffer;
        NPRPC_QUIC_DEBUG_LOG(
          std::format("Server datagram size={}", buf->Length));
...
void QuicServerConnection::handle_stream_event(
  HQUIC stream, QUIC_STREAM_EVENT* event)
{
  NPRPC_QUIC_DEBUG_LOG(
  ...
  switch (event->Type) {
    case QUIC_STREAM_EVENT_RECEIVE: {
      ...
      for (uint32_t i = 0; i < event->RECEIVE.BufferCount; ++i) {
        auto& buf = event->RECEIVE.Buffers[i];
        NPRPC_QUIC_DEBUG_LOG(
          std::format("Server received {} bytes", buf.Length));
        receive_buffer_.insert(
          receive_buffer_.end(), buf.Buffer, buf.Buffer + buf.Length);
      }
...
void QuicServerConnection::process_receive_buffer()
{
  ...
  while (receive_buffer_.size() >= 4) {
    ...
    uint32_t total_size = payload_size + 4;
    NPRPC_QUIC_DEBUG_LOG(std::format(
      "process_receive_buffer: payload_size={}, total_size={}, buffer_size={}",
      payload_size,
      total_size,
...
void QuicListener::start(
  uint16_t port, AcceptCallback callback)
{
  auto& quic = QuicApi::instance();
...
void QuicListener::add_connection(
...
void QuicListener::remove_connection(
  QuicServerConnection* conn)
{
  std::lock_guard lock(connections_mutex_);
  connections_.erase(
...
void QuicListener::handle_listener_event(
                          ...
                          public std::enable_shared_from_this<QuicServerSession>
{
 ...
 public:
  ...
  {
    ctx_.remote_endpoint = EndPoint(EndPointType::Quic,
                          ...
                          public std::enable_shared_from_this<QuicClientSession>
{
 ...
 public:
  ...
  virtual void send_receive(
    flat_buffer& buffer, uint32_t timeout_ms) override
  {
    if (!connected_) {
  ...
  QuicClientSession(
    const EndPoint& endpoint, boost::asio::io_context& ioc)
...
NPRPC_API void init_quic(
  boost::asio::io_context& ioc)
{
  if (g_cfg.listen_quic_port == 0) return;

  try {
    g_quic_listener = std::make_shared<QuicListener>(
      ioc, g_cfg.quic_cert_file, g_cfg.quic_key_file);
