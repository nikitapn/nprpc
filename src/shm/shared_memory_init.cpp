...
void init_shared_memory_listener(
  boost::asio::io_context& ioc)
{
  ...
  NPRPC_LOG_INFO("Starting shared memory listener: {}", g_server_listener_uuid);
  ...
  g_shared_memory_listener = std::make_shared<SharedMemoryListener>(
    ...
    [&ioc](std::unique_ptr<SharedMemoryChannel> channel) {
      ...
      auto session =
        create_shared_memory_server_session(ioc, std::move(channel));
  ...
  NPRPC_LOG_INFO("Shared memory listener started successfully");
}
