...
UdpConnection::UdpConnection(
  boost::asio::io_context& ioc, const endpoint_type& remote_endpoint)
    : ioc_(ioc),
  ...
  flat_buffer&& buffer)
{
  ...
  if (g_cfg.debug_level >= DebugLevel::DebugLevel_EveryMessageContent) {
    ...
    NPRPC_LOG_INFO("[UDP] send() called with {} bytes to {}:{}",
                   data.size(),
                   remote_endpoint_.address().to_string(),
                   remote_endpoint_.port());
  }
...
void UdpConnection::do_send()
{
  ...
  if (g_cfg.debug_level >= DebugLevel::DebugLevel_EveryMessageContent) {
    NPRPC_LOG_INFO("[UDP] Sending {} bytes to {}:{}",
                   data.size(),
                   remote_endpoint_.address().to_string(),
                   remote_endpoint_.port());
  }
  ...
  uint32_t         max_retries)
{
  ...
  pending.timer->async_wait(
    [this, self = shared_from_this(), request_id, &buffer](
  ...
  socket_.async_send_to(
    ...
    [this, self = shared_from_this(), request_id](
      ...
      if (ec && g_cfg.debug_level >= DebugLevel::DebugLevel_Critical) {
        NPRPC_LOG_ERROR(
          "[UDP] Send error for request_id={}: {}", request_id, ec.message());
      }
    });
}
...
void UdpConnection::handle_response(
  size_t bytes_received)
{
  ...
  if (g_cfg.debug_level >= DebugLevel::DebugLevel_EveryCall) {
    NPRPC_LOG_INFO("[UDP] Received response request_id={} size={}",
                   request_id,
                   bytes_received);
  }
...
void UdpConnection::do_retransmit(
  uint32_t request_id)
{
  auto it = pending_calls_.find(request_id);
  if (it == pending_calls_.end()) {
