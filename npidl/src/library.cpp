...
struct Token {
  ...
  bool operator==(TokenId id) const noexcept { return this->id == id; }
  ...
  bool operator!=(TokenId id) const noexcept { return !(operator==(id)); }

  bool operator!=(char id) const noexcept { return !(operator==(id)); }
...
template <typename Key, typename T, size_t Size> struct Map {
  const std::pair<Key, T> (&items_)[Size];

  constexpr std::optional<std::pair<Key, T>> find(const Key& key) const noexcept
  {
    const auto it = std::find_if(items_, items_ + Size, [&key](const auto& x) {
      return x.first == key;
    });
...
class Lexer : public ILexer
{
  ...
  char look() const noexcept { return *(ptr_ + 1); }
  ...
  Token read_string(int tok_line, int tok_col)
  {
    ...
    static constexpr Map<std::string_view, TokenId,
                         sizeof(alphabet) /
                             sizeof(std::pair<std::string_view, TokenId>)>
        map{alphabet};
    ...
    if (str == "true")
      return Token(TokenId::Number, "1", tok_line, tok_col);
    if (str == "false")
      return Token(TokenId::Number, "0", tok_line, tok_col);
...
public:
  ...
  ISourceProvider& get_source_provider() override { return source_provider_; }
  ...
  Token tok() override
  {
    ...
    switch (cur()) {
    case '\0':
      return Token(TokenId::Eof, tok_line, tok_col);
    case ':':
      next();
      if (cur() == ':') {
        next();
...
#define TOKEN_FUNC(x, y)                                                       \
  case y:                                                                      \
    next();                                                                    \
    return Token(TokenId::x, tok_line, tok_col);
      ONE_CHAR_TOKENS()
#undef TOKEN_FUNC
...
template <typename T, size_t MaxSize> class Queue
{
  T* begin_;
  T* end_;
  size_t size_;
...
class Parser : public IParser
{
  ...
  using attributes_t =
      boost::container::small_vector<std::pair<std::string, std::string>, 2>;
  ...
  Token match(char id) { return match(static_cast<TokenId>(id)); }
  ...
  template <class MemFn, typename... Args> bool check(MemFn Pm, Args&&... args)
  {
  ...
  AstNumber parse_number(const Token& tok)
  {
    ...
    if (str.size() > 2 && str[0] == '0' && str[1] == 'x') {
      auto result =
          std::from_chars(str.data() + 2, str.data() + str.size(), i64, 16);
      if (result.ec != std::errc()) {
        throw_error("Hex number.std::from_chars returned: " +
      ...
      if (str.back() == 'f') {
        auto result =
            std::from_chars(str.data(), str.data() + str.size(), flt32);
        if (result.ec != std::errc()) {
          throw_error("Float32 number.std::from_chars returned: " +
                      std::make_error_code(result.ec).message());
        }
        return AstNumber{flt32, NumberFormat::Decimal};
      } else {
        auto result =
            std::from_chars(str.data(), str.data() + str.size(), flt64);
        if (result.ec != std::errc()) {
          throw_error("Float64 number.std::from_chars returned: " +
                      std::make_error_code(result.ec).message());
        }
      ...
      auto result =
          std::from_chars(str.data(), str.data() + str.size(), i64, 10);
      if (result.ec != std::errc()) {
        throw_error("Integer number.std::from_chars returned: " +
                    std::make_error_code(result.ec).message());
      }
  ...
  // Helper to set position on AST node (when we have start token and know
  // current end)
  template <typename T>
  ...
  bool field_decl(AstFieldDecl*& field)
  {
    ...
    SourcePosition end(field_name.line,
  ...
  bool arg_decl(AstFunctionArgument& arg, Token& start_token, Token& type_token)
  {
    ...
    if (!((arg_name == TokenId::Identifier || arg_name == TokenId::Message) &&
          maybe(optional, &Parser::one, TokenId::Optional) &&
          peek() == TokenId::Colon))
      return false;
  ...
  {
    if (!(peek() == TokenId::Hash && peek() == TokenId::Identifier &&
          tokens_.back().name == "version"))
      return false;
  ...
  bool const_decl()
  {
    if (peek() != TokenId::Const)
      return false;
    flush();
    auto var_name = match(TokenId::Identifier);
    match('=');
    auto var_value = parse_number(match(TokenId::Number));
    match(';');
  ...
  bool namespace_decl()
  {
    if (peek() != TokenId::Namespace)
      return false;
    flush();

    auto name = match(TokenId::Identifier);
    match('{');
  ...
  bool import_decl()
  {
    ...
    import->path_end_col = import_path_tok.col +
                           (int)import_path_tok.name.length() +
                           2; // +2 for quotes

    // Use injected resolver to resolve import path
    auto resolved = import_resolver_.resolve_import(import->import_path,
                                                    ctx_.get_file_path());
    ...
    if (resolved) {
      ...
      // Check if we should parse this import (depends on resolver
      // strategy)
      if (import_resolver_.should_parse_import(*resolved)) {
        ...
        try {
          ...
          Parser import_parser(import_lexer, ctx_, builder_, import_resolver_,
                               error_handler_);

          // Parse the imported file
          import_parser.parse();

          // FileContextGuard destructor automatically pops file
          // context
        } catch (const parser_error& e) {
          import->resolved = false;
          import->error_message =
              std::string("Error parsing imported file: ") + e.what();
      ...
      error_handler_.handle_error(parser_error(
          ctx_.current_file_path(), import_tok.line, import_tok.col,
...

    {
      ...
      if (peek() == TokenId::Colon) {
        ...
        for (;;) {
          ...
          if (auto type = ctx_.nm_cur()->find_type(name, false);
      ...
      if (check(&Parser::function_decl, std::ref(f))) {
        ...
        for (const auto& a : fn_attr) {
          if (a.first == "unreliable")
            ...
            throw_error("Unknown attribute: " + a.first + " for function " +
                        f->name);
        }
  ...
  bool alias_decl()
  {
    ...
    if (!((start_tok = peek()) == TokenId::Alias &&
  ...
  // (interface_decl | struct_decl) | enum_decl | alias_decl | module_decl |
  // ';'
  bool stmt_decl()
  {
    return (check(&Parser::import_decl) || check(&Parser::const_decl) ||
            check(&Parser::namespace_decl) ||
            check(&Parser::something_that_could_have_attributes) ||
            check(&Parser::enum_decl) || check(&Parser::alias_decl) ||
  ...
  void synchronize()
  {
    ...
    // Good sync points: semicolon, closing brace, namespace, interface,
    // struct
    panic_mode_ = false;
    flush();
    ...
    while (true) {
      ...
      if (t == TokenId::Semicolon || t == TokenId::BracketClose ||
          t == TokenId::Eof) {
        break;
      }
      ...
      Token next = peek();
      if (next == TokenId::Namespace || next == TokenId::Interface ||
          next == TokenId::Message || next == TokenId::Exception ||
          next == TokenId::Enum || next == TokenId::Alias ||
          next == TokenId::Const || next == '[') { // Attributes
        break;
      }
      flush();
    }
  ...
  template <typename Fn> bool try_parse(Fn&& fn, const char* error_msg)
  {
    if (!error_handler_.should_continue_after_error()) {
      return fn();
    }
    ...
    try {
      ...
      record_error(
          parser_error(ctx_.current_file_path(), e.line, e.col, e.what()));
      synchronize();
      return false;
    }
  }
...
std::tuple<std::shared_ptr<LspSourceProvider>,
           std::unique_ptr<LspImportResolver>,
           std::unique_ptr<LspErrorHandler>,
           std::unique_ptr<ILexer>,
           std::unique_ptr<IParser>>
ParserFactory::create_lsp_parser(
    Context& ctx,
    builders::BuildGroup& builder,
    std::shared_ptr<LspSourceProvider> source_provider)
{
  if (!source_provider) {
...
std::tuple<std::shared_ptr<InMemorySourceProvider>,
           std::unique_ptr<LspImportResolver>,
           std::unique_ptr<LspErrorHandler>,
           std::unique_ptr<ILexer>,
           std::unique_ptr<IParser>>
ParserFactory::create_test_parser(Context& ctx,
                                  builders::BuildGroup& builder,
                                  const std::string& content)
{
