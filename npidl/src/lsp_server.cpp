...
void DocumentManager::change(const std::string& uri,
                             const std::string& text,
                             int version)
{
...
DocumentManager::parse_and_get_diagnostics(npidl::WorkspaceManager& workspace,
                                           Document& doc)
{
  ...
  npidl::PositionIndexBuilder index_builder(project->position_index,
                                            *project->ctx);
  index_builder.build();
    ...
    while (std::getline(stream, line)) {
      if (current_line == err.line - 1) {
        ...
        if (pos > 0 && pos <= static_cast<int>(line.size())) {
          ...
          // Now find the start of the token (alphanumeric or
          // underscore)
          int token_end = pos;
          while (pos > 0 &&
          ...
          if (pos < token_end) {
            ...
            // No token found backwards, try forward from error
            // position
            pos = err.col - 1;
            if (pos >= 0 && pos < static_cast<int>(line.size())) {
              int token_start = pos;
...
std::optional<std::string> LspServer::read_message()
{
  ...
  if (!std::cin) {
    std::clog << "Failed to read " << content_length << " bytes from stdin"
              << std::endl;
    return std::nullopt;
  }

  std::clog << "Successfully read message of " << content_length << " bytes"
            << std::endl;
  return content;
}
...
void LspServer::send_error(const glz::generic& id,
                           int code,
                           const std::string& message)
{
  std::string error_obj =
      glz::write_json(glz::obj{"code", code, "message", message})
          .value_or("{}");
...
void LspServer::handle_initialize(const glz::generic& id,
...
void LspServer::handle_did_open(const glz::raw_json& params)
{
  ...
  if (error) {
    std::cerr << "Error parsing didOpen params: "
              << glz::format_error(error, params.str) << std::endl;
    return;
  }

  std::clog << "Document opened: " << open_params.textDocument.uri << std::endl;

  documents_.open(open_params.textDocument.uri, open_params.textDocument.text,
...
void LspServer::handle_did_change(const glz::raw_json& params)
{
  ...
  if (error) {
    std::cerr << "Error parsing didChange params: "
...
void LspServer::handle_did_close(const glz::raw_json& params)
{
  ...
  if (error) {
    std::cerr << "Error parsing didClose params: "
              << glz::format_error(error, params.str) << '\n';
    return;
  }

  std::clog << "Document closed: " << close_params.textDocument.uri
            << std::endl;
...
void LspServer::handle_hover(const glz::generic& id,
                             const glz::raw_json& params)
{
  ...
  if (error) {
    std::cerr << "Error parsing hover params: "
              << glz::format_error(error, params.str) << '\n';
    send_response(id, "null");
    return;
  }
  ...
  std::clog << "Project found. Index has " << project->position_index.size()
            << " entries" << '\n';
  std::clog << "Index finalized: "
            << (project->position_index.is_finalized() ? "yes" : "no") << '\n';
  ...
  std::clog << "Looking for node at 1-based position " << line << ":" << col
            << std::endl;
  ...
  if (!entry) {
    ...
    for (size_t i = 0; i < std::min(size_t(10), entries.size()); ++i) {
      std::clog << "  Entry " << i << ": lines " << entries[i].start_line << "-"
                << entries[i].end_line << ", cols " << entries[i].start_col
                << "-" << entries[i].end_col << '\n';
    }
    send_response(id, "null");
    return;
...
std::string
LspServer::create_hover_content(const npidl::PositionIndex::Entry* entry)
{
  ...
  case NodeType::Parameter: {
    ...
    md << "Direction: "
       << (param->modifier == npidl::ArgumentModifier::In ? "in" : "out")
       << "\n";
    break;
  }
                                            ...
                                            const glz::raw_json& params)
{
  ...
  if (error) {
    std::cerr << "Error parsing semantic tokens params: "
              << glz::format_error(error, params.str) << '\n';
    send_response(id, "null");
    return;
  }

  std::clog << "Semantic tokens request for " << token_params.textDocument.uri
            << std::endl;
  ...
  std::clog << "Generating semantic tokens. Index has "
            << project->position_index.size() << " entries" << std::endl;
  ...
  for (const auto& entry : entries) {
    ...
    uint32_t token_modifiers =
        TM_DECLARATION; // All indexed entries are declarations
  ...
  std::clog << "Generated " << (data.size() / 5) << " semantic tokens"
            << std::endl;
...
void LspServer::handle_definition(const glz::generic& id,
                                  const glz::raw_json& params)
{
  lsp::TextDocumentPositionParams pos_params;
  ...
  auto error = glz::read_json(pos_params, params.str);
  if (error) {
    std::cerr << "Error parsing definition params: "
              << glz::format_error(error, params.str) << '\n';
    send_response(id, "null");
    return;
  }
  ...
  std::clog << "Looking for definition at 1-based position " << line << ":"
            << col << std::endl;
                                       ...
                                       const glz::raw_json& params)
{
  ...
  if (error) {
    std::cerr << "Error parsing documentSymbol params: "
              << glz::format_error(error, params.str) << '\n';
...
void LspServer::handle_debug_positions(const glz::generic& id,
                                       const glz::raw_json& params)
{
  ...
  if (error) {
    std::cerr << "Error parsing debug positions params: "
              << glz::format_error(error, params.str) << '\n';
    send_response(id, "null");
    return;
  }
  ...
  for (size_t i = 0; i < entries.size(); ++i) {
    ...
    result << "  Name: " << name << "\n";
    result << "  Position: " << entry.start_line << ":" << entry.start_col
...
void LspServer::publish_diagnostics(
    const std::string& uri, const std::vector<lsp::Diagnostic>& diagnostics)
{
  std::string params =
      glz::write_json(glz::obj{"uri", uri, "diagnostics", diagnostics})
          .value_or("{}");
