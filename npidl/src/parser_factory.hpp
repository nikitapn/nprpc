...
public:
  ...
  static std::tuple<std::unique_ptr<FileSystemSourceProvider>,
  ...
  static std::tuple<std::shared_ptr<LspSourceProvider>, // Shared across
                                                        // documents
  ...
  create_lsp_parser(
      ...
      std::shared_ptr<LspSourceProvider> source_provider = nullptr);

  // Create parser for testing with in-memory content (collects errors)
  static std::tuple<std::shared_ptr<InMemorySourceProvider>,
                    std::unique_ptr<LspImportResolver>,
                    std::unique_ptr<LspErrorHandler>,
                    std::unique_ptr<ILexer>,
                    std::unique_ptr<IParser>>
  create_test_parser(Context& ctx,
                     builders::BuildGroup& builder,
