...
public:
  ...
  lexical_error(std::string_view _file_path,
                int _line,
                int _col,
                const char* msg)
...
{
public:
  parser_error(std::string_view _file_path,
               int _line,
               int _col,
               const char* msg)
...
public:
  ...
  resolve_import(const std::string& import_path,
                 const std::filesystem::path& current_file_path) = 0;

  // Check if we should parse this import file
  // (Used to prevent circular imports and duplicate parsing)
  virtual bool
  should_parse_import(const std::filesystem::path& resolved_path) = 0;
};
...
public:
  CompilationBuilder&
  set_input_files(const std::vector<std::filesystem::path>& input_files);
