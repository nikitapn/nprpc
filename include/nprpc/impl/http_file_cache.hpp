...
public:
    ...
    bool load(const std::filesystem::path& path, size_t mmap_threshold = 64 * 1024);
    ...
    void acquire() const noexcept { active_refs_.fetch_add(1, std::memory_order_relaxed); }
    void release() const noexcept { active_refs_.fetch_sub(1, std::memory_order_release); }
    int32_t active_refs() const noexcept { return active_refs_.load(std::memory_order_acquire); }
...
public:
    ...
    const std::shared_ptr<const CachedFile>& get() const noexcept { return file_; }
...
struct HttpFileCacheConfig {
    ...
    std::chrono::seconds mtime_check_interval{60};  // Re-check file mtime every 60s
};
