...
template <class type>
type& operator<<(type& _Ostr, const nprpc::clr::color &color)
{
#ifdef _WIN32
    if (nprpc::clr::internal::old_color_attrs == static_cast<WORD>(-1))
    {
        // Store the current text color
        CONSOLE_SCREEN_BUFFER_INFO buffer_info;
        GetConsoleScreenBufferInfo(GetStdHandle(STD_OUTPUT_HANDLE), &buffer_info);
        nprpc::clr::internal::old_color_attrs = buffer_info.wAttributes;
    }
#endif
    if (color == nprpc::clr::color::reset)
    {
#ifdef _WIN32
        _Ostr.flush();
        // Restores the text color.
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), nprpc::clr::internal::old_color_attrs);
#elif __unix__
...
#elif __unix__
        _Ostr << "\033[0;3" << nprpc::clr::internal::GetAnsiColorCode(color) << "m";
#endif
    }
    return _Ostr;
}
