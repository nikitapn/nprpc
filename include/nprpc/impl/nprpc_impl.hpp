...
class CommonConnection
{
 ...
 protected:
  ...
  void add_work(
    std::shared_ptr<IOWork> w)
  {
    boost::asio::post(derived()->get_executor(),
                      [w {std::move(w)}, this]() mutable {
 ...
 public:
    ...
    const flat_buffer& buf, WriteHandler&& handler)
  {
    ...
    boost::asio::async_write(
      socket_, buf.cdata(), std::forward<WriteHandler>(handler));
  }
...
class RpcImpl : public Rpc
{
 ...
 public:
   ...
   * retransmit)
   * @param completion_handler Called on completion with error code and response
   * buffer
   * @param timeout_ms Timeout per attempt in milliseconds
   * @param max_retries Maximum retransmit attempts before failure
   */
   ...
   * @param buffer Message buffer to send (moved)
   */
  NPRPC_API void send_unreliable(const EndPoint& endpoint,
                                 flat_buffer&&   buffer);
...
class PoaImpl : public Poa, public std::enable_shared_from_this<PoaImpl>
{
  // User-supplied IDs: simple array indexed by object_id (0..max_objects_-1),
  // lock-free
  struct UserObjects {
    std::unique_ptr<std::atomic<ObjectServant*>[]> slots;
    explicit UserObjects(
 ...
 public:
  auto get_lifespan() const noexcept { return pl_lifespan_; }
...
inline void make_simple_answer(
  SessionContext& ctx, MessageId id, uint32_t request_id = 0)
{
  assert(id == MessageId::Success || id == MessageId::Error_PoaNotExist ||
...
NPRPC_API void dump_message(flat_buffer& buffer, bool rx);
...
inline int handle_standart_reply(
  flat_buffer& buf)
{
  if (buf.size() < sizeof(impl::flat::Header)) throw ExceptionBadInput();
