...
public:
   ...
   *
   * For blocking callers - buffer passed by reference (no copy for initial
   * send). Buffer is copied on first timeout for retransmit.
   *
   * @param buffer Message buffer (reference - caller blocks, so no copy
   * needed)
...
private:
  ...
  struct PendingCall {
    flat_buffer request; // Copy of request for retransmit (lazy - copied on
                         // first timeout)
