...
public:
  uint64_t add(const T &val) noexcept {
    ...
    for (;;) {
      ...
      // Use acq_rel: acquire to read next pointer, release to publish the claimed slot
      if (tail_ix_.compare_exchange_weak(old_free, new_free, std::memory_order_acq_rel, std::memory_order_relaxed)) break;
    }
  ...
  std::remove_pointer_t<T> *get(uint64_t id) noexcept {
    ...
    auto &item = data(idx);
    if (item.gix.load(std::memory_order_relaxed) != generation_index(id)) return nullptr;
