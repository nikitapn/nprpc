...
template <typename T>
concept JsonArchive =
    std::is_same_v<T, json_iarchive> || std::is_same_v<T, json_oarchive>;

template <typename Archive, typename T> void serialize(Archive& ar, T& obj)
{
  obj.serialize(ar);
}

template <typename Archive, typename T> void serialize_free(Archive& ar, T& obj)
{
  serialize(ar, obj);
}
...
template <typename Archive> void serialize_free(Archive& ar, std::string& obj)
{
...
// Optimized serialization for contiguous containers (std::vector, std::array,
// std::string)
...
// General serialization for iterator-based containers (std::list, std::set,
// std::map, etc.)
...
struct bool_t {
  struct save {
    template <typename Archive> static void invoke(Archive& ar, const bool obj)
    {
      ar.save_bool(obj);
    }
  };
  struct load {
    template <typename Archive> static void invoke(Archive& ar, bool& obj)
    {
      ar.load_bool(obj);
    }
  };
...
class basic_oarchive_abstract : public basic_archive_abstract
{
};

class basic_iarchive_abstract : public basic_archive_abstract
{
};
...
template <typename Archive> class basic_archive
{
public:
  Archive* This() noexcept { return static_cast<Archive*>(this); }

  template <typename T> void save(T& obj)
  {
    using type = std::conditional_t<
        std::is_same_v<T, bool>, bool_t,
        std::conditional_t<
            std::is_fundamental_v<T>, fundamental_t,
            std::conditional_t<
                std::is_enum_v<T>, fundamental_t,
                std::conditional_t<std::is_base_of_v<nvp_base, T>, nvp_t,
                                   default_t>>>>;
    type::save::invoke(*This(), obj);
  }

  template <typename T> void load(T& obj)
  {
    using type = std::conditional_t<
        std::is_fundamental_v<T>, fundamental_t,
        std::conditional_t<std::is_enum_v<T>, fundamental_t,
                           std::conditional_t<std::is_base_of_v<nvp_base, T>,
                                              nvp_t, default_t>>>;
    type::load::invoke(*This(), obj);
  }

  template <typename T> void save_sequence(const T* data, unsigned int size)
  {
    for (unsigned int i = 0; i < size; ++i) {
...
class basic_oarchive : public basic_archive<Archive>,
...
public:
  ...
  template <typename T> Archive& operator<<(const T& obj)
  {
...
template <typename Archive>
class basic_iarchive : public basic_archive<Archive>,
...
public:
  ...
  template <typename T> Archive& operator>>(T& obj)
  {
