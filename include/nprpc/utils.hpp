...
template <typename T>
struct is_raw_or_smart_pointer<const T*> : std::true_type {
};
template <typename T>
struct is_raw_or_smart_pointer<std::unique_ptr<T>> : std::true_type {
};
template <typename T>
struct is_raw_or_smart_pointer<const std::unique_ptr<T>> : std::true_type {
};
template <typename T>
struct is_raw_or_smart_pointer<std::shared_ptr<T>> : std::true_type {
};
template <typename T>
struct is_raw_or_smart_pointer<const std::shared_ptr<T>> : std::true_type {
};

template <typename T, typename U> struct wrapper1 {
...
template <typename T> struct wrapper1<T, std::true_type> {
  typedef std::conditional_t<std::is_const_v<T>, const T, T> _T;
